...
// robot setup: align to tile edge  with 1 channel distance to the front
void shootRollFar() {
  Drivetrain.setHeading(180, degrees);
  Drivetrain.turnFor(ANGLE_HARD, degrees, 50, velocityUnits::pct, false);
  shootDisks();
  quickTurn(-ANGLE_HARD, 50);
  double d = getDistanceAverage(frontDistance, 59, 5);
  if (d < 0)
    d = 59;

  Drivetrain.driveFor(reverse, d - DISTANCE_HARD, inches, 50,
                      velocityUnits::pct);
  slideToRoller();
  spinRoll();
}
...
void afterFar() {
  ...
  double d = getDistanceAverage(frontDistance, 57, 5);
  if (d < 0)
    return;
...
int shootString() {
  ...
  while (true) {
    ...
    if (heading > 90 + 40 + 180 && heading <= 90 + 50 + 180) {
      piston.set(true);
      Controller1.rumble("..");
      Brain.Timer.clear();
      while (true) {
